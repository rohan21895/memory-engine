...
Useful flags: `--stages ingest,analysis`, `--rescan`, `--reanalyze-faces`,
`--icc-profile PATH`, `--album-photos N`, `--quiet`.
...
All are in Codex's territory or in the shared `contracts/`, taken under issue
#48, and each is **a blocking defect the path could not work around**:
