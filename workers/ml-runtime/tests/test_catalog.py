...
class TestModelCatalog(CatalogFixture):
    def test_current_registry_configs_match_their_raw_byte_pins(self) -> None:
        ...
        expected = {entry["model_id"]: entry["config_blake3"] for entry in registry["entries"]}
        self.assertEqual(6, len(inspections))
        for inspection in inspections:
            with self.subTest(model=inspection.model_id):
                self.assertEqual(expected[inspection.model_id], inspection.config_blake3)
                self.assertEqual(
    ...
    def test_development_opt_in_allows_unpinned_weight_files(self) -> None:
        ...
        self.assertTrue(all(item.unloadable_reason is None for item in catalog.inspect_all()))
    ...
    def test_semantically_equal_reformatted_config_fails_its_byte_pin(self) -> None:
        ...
        self.assertEqual("UNLOADABLE_REASON_CONFIG_MISMATCH", inspected.unloadable_reason)
    ...
    def test_catalog_delegates_every_decision_to_shared_load_gate(self) -> None:
        ...
        self.assertEqual("UNLOADABLE_REASON_CONFIG_INVALID", inspected[0].unloadable_reason)
    ...
    def test_registry_paths_cannot_escape_the_models_or_weights_roots(self) -> None:
        ...
        self.assertEqual("UNLOADABLE_REASON_CONFIG_MISSING", inspected.unloadable_reason)
