...
| Body pose | RTMO-m body7 (MMPose), via rtmlib | ONNX | **Code AND weights Apache-2.0** — MMPose maintainers confirm commercial permission (issues [#2106](https://github.com/open-mmlab/mmpose/issues/2106) / [#2393](https://github.com/open-mmlab/mmpose/issues/2393)) | LIKELY OK | no | 17 COCO keypoints → the album's pose-variety axis. Config `models/configs/rtmo-m-body7.json`; stage `services/pipeline/…/stages/pose.py`; geometry `packages/album-engine/…/pose.py`. **Not loaded by `workers/ml-runtime`** — `rtmlib` fetches and caches the ONNX under `~/.cache/rtmlib`, so the config is provenance, not a host binding, and its pre/post fields are unverified against the graph. **Open TODO, tracked in the config's `training_data`:** the `body7` checkpoint mixes COCO (CC-BY-4.0, clean) with AI Challenger, CrowdPose, MPII, sub-JHMDB, Halpe and PoseTrack18 — only COCO's terms are confirmed. Audit the AI Challenger / Halpe terms, or repin a COCO-only checkpoint, before commercial ship. Full candidate sweep and fallbacks (RTMPose-m + detector; DETRPose-M challenger) in `docs/pose-model-proposal.md`. |
