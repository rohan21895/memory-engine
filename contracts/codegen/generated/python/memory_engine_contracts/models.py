...
class FaceRecord(ContractModel):
    ...
    face_id: Blake3Hash = Field(
        description="BLAKE3 over (media_id, frame_time, quantised bbox, detector model_id+version). Content-addressed, so re-running the same detector on the same frame produces the same id and re-detection is idempotent. Changing detector version deliberate...",
    )
