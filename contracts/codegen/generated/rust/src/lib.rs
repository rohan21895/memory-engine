...
pub struct FaceRecord {
    ...
    /// over the 10000 half-quantum decimal literals in [0,1] the two readings disagree on
    /// 4419 of them, and over two million uniformly random doubles they disagree on none --
    /// which is why a suite built from random boxes cannot find this, and why the vector
    /// table carries `bbox-half-quantum-only-in-binary64` specifically. Binary64 is pinned
    /// because it is what a producer doing float arithmetic computes without trying, and
    /// because it is what both existing implementations already do; a third writer reaching
    /// for exact rational arithmetic to be 'more correct' would silently issue a second id
    /// for a face that already has one. `rotation_deg` does NOT participate, which is why
    /// Detection pins it to 0.
    ///
