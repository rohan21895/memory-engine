...
export interface ClassifierPin {
  ...
  /**
   * WHICH COLUMN OF THE CLASSIFIER'S OUTPUT VECTOR IS WHICH CLASS, as the producer applied
   * it.
   *
   ...
   * longer visible -- and if it was applied wrongly, nothing here or downstream can tell.
   * Transpose two columns and every breastfeeding photograph classifies as `explicit`,
   * every score is in range, every count adds up, the manifest validates, and no test
   * fails. That is the whole reason this field exists: it is the only place the mapping is
   * written down where a machine can check it.
   *
   * It is a `const`, so the only valid value is the contract's own order and a transposed
   * manifest is REJECTED BY VALIDATION rather than by review. Const does not make it
   * pointless to write: the producer must copy it out of the config that governed the run,
   * and `models/schema/model-config.schema.json` pins the same three strings in the same
   * order on the safety classifier's output. A config whose `class_order` was edited can
   * no longer produce a manifest that validates.
   *
...
export interface SafetyClearance {
   ...
   * SERIALISED BYTES of this document with manifest_id and decision removed. Bytes rather
   * than a re-serialisation.' There are no such bytes: you cannot delete two keys from a
   * byte string without re-serialising it, so every implementation would have invented its
   * own canonical form -- which is the exact divergence the sentence was trying to
   * prevent. models/policy/digest.py can hash bytes because a config file IS the artifact;
   * a manifest body is a projection of one, so it has to be constructed.
   *
   ...
   * `contracts/vectors/safety-clearance-manifest-id.json` pins the exact pre-image bytes,
   * and both the Python and TypeScript implementations are checked against it -- a digest
   * mismatch says only that something diverged, the pre-image says which field did.
   *
   * WHAT EXCLUDING `decision` MEANS, STATED SO NOBODY HAS TO REDISCOVER IT. Two manifests
   * that differ ONLY in their decision block hash to the SAME manifest_id -- they describe
   * the same publication, and the decision is a derived assertion about it rather than
   * part of it. That is safe exactly as long as every verifier RECOMPUTES the decision
   * from `items` and denies on disagreement, which is what `ClearanceDecision` already
   * requires and what both implementations do. It is not safe if any consumer ever reads
   * `cleared_for_publication` and believes it. The vectors file carries this pair as
   * `same-body-different-decision` so the property is tested rather than assumed.
   */
